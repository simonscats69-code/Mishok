...
class Stats:
    ...
    def daily_activity(self, user_id, days=7):
        stats = get_detailed_stats(user_id, days); daily = stats.get('daily_activity',{})
        result = {}
        for date_str, count in daily.items():
            try: date_obj = datetime.fromisoformat(date_str); result[date_obj.strftime("%d.%m")] = count
            except: result[date_str] = count
        ...
        sorted_items = sorted(result.items(), key=lambda x: datetime.strptime(x[0], "%d.%m").date())
    ...
    def favorite_time(self, user_id):
        ...
        if not any(hours): return "📊 Пока нет данных"
        max_hour = hours.index(max(hours)); max_count = max(hours)
        blocks = [(0,5,"🌙 Ночью (0-6)",sum(hours[0:6])),(6,11,"🌅 Утром (6-12)",sum(hours[6:12])),(12,17,"☀️ Днём (12-18)",sum(hours[12:18])),(18,23,"🌆 Вечером (18-24)",sum(hours[18:24]))]
        best = max(blocks, key=lambda x: x[3])
        if 0<=max_hour<6: desc = "ночью 🌙"
        elif 6<=max_hour<12: desc = "утром 🌅"
        elif 12<=max_hour<18: desc = "днём ☀️"
        else: desc = "вечером 🌆"
    ...
    def comparison(self, user_id):
        ...
        if not data['total_users']: return {'total_users':0,'avg_shleps':0,'percentile':0,'rank':1,'user_shleps':0,'median_shleps':0}
        _, user_shleps, _ = get_user_stats(user_id)
        counts = data['user_counts']; better = sum(1 for c in counts if c > user_shleps)
        rank = better + 1; perc = ((data['total_users'] - better) / data['total_users'] * 100)
        median = calculate_median(counts) if counts else 0; avg = sum(counts)/len(counts) if counts else 0
        return {'total_users':data['total_users'],'avg_shleps':round(avg,1),'median_shleps':round(median,1),'percentile':round(perc,1),'rank':rank,'user_shleps':user_shleps}
    ...
    def global_trends(self):
        ...
        else: trends['avg_per_user_24h'] = 0
