...
Для production развертывания используйте Docker с персистентным volume для сохранения данных:
...
services:
  mishok-bot:
    ...
    environment:
      ...
      - DATA_PATH=/app/data  # Важно: данные внутри контейнера
    volumes:
      - ./data:/app/data     # Важно: volume для персистентности
...
# Просмотр логов
docker-compose logs -f mishok-bot
```

**Важно для сохранения данных:**
- Установите `DATA_PATH=/app/data` в environment
- Настройте volume: `./data:/app/data`
- Без volume данные будут теряться при перезапуске контейнера
...
**Решение:**
1. **В Docker:** Убедитесь, что настроен volume: `./data:/app/data`
2. **В переменных окружения:** Установите `DATA_PATH=/app/data`
3. **Проверьте логи:** Бот должен логировать "Данные загружены" при старте
4. **Резервное копирование:** Используйте `/backup` для создания бэкапов

### Проблема: Бот не удаляет сообщения забаненных пользователей
**Решение:**
1. **Проверьте права бота:** Бот должен быть администратором чата с правом "Удалять сообщения"
2. **Проверьте логи:** Ищите "check_banned_messages вызвана" в логах
3. **Тип чата:** Должен быть supergroup, не private
4. **Webhook vs Polling:** Убедитесь, что используется polling (по умолчанию)

### Проблема: Бот не реагирует на сообщения в чате
**Решение:**
1. **Бот в чате:** Добавьте бота в группу
2. **Webhook:** Если используется webhook, проверьте его настройку
3. **Логи:** Проверьте логи на ошибки подключения
4. **Токен:** Убедитесь, что BOT_TOKEN правильный
